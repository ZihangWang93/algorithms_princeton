...
/******************************************************************************
 *  Compilation:  javac TrieSETR.java
 *  Execution:    java TrieSETR < words.txt
...
/**
 * The {@code TrieSETR} class represents an ordered set of strings over the extended ASCII alphabet.
 * It supports the usual <em>add</em>, <em>contains</em>, and <em>delete</em> methods. It also
 * provides character-based methods for finding the string in the set that is the <em>longest
 * prefix</em> of a given prefix, finding all strings in the set that <em>start with</em> a given
 * prefix, and finding all strings in the set that <em>match</em> a given pattern.
 * <p>
 * This implementation uses a 256-way trie. The <em>add</em>, <em>contains</em>, <em>delete</em>,
 * and
...
public class TrieSETR implements Iterable<String> {
    private static final int R = 26;        // Just 26 english characters
    ...
    public boolean containsPrefix(String preflix) {
        if (preflix == null) {
            return false;
        }
        Node node = root;
        ...
        for (int i = 0; i < preflix.length(); i++) {
            char c = preflix.charAt(i);
            if (node.next[c - 65] == null) {
                return false;
            }
            node = node.next[c - 65];
        }
    ...
    /**
     * Unit tests the {@code TrieSETR} data type.
     *
